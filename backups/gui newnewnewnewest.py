...
#            if True:
            ...
            #if round_cnt % 2 == 0:
            if True:
                print("after players's move: "+str(self.game.calculate_reward(
                        self.agents[1-self.playing_agent],
                        self.game.get_game_phase(self.agents[1-self.playing_agent]),
                        self.agents[self.playing_agent],
                        self.game.get_game_phase(self.agents[self.playing_agent])
                        )))  
                ...
                print("after computer's move: "+str(self.game.calculate_reward(
                        self.agents[self.playing_agent],
                        self.game.get_game_phase(self.agents[self.playing_agent]),
                        self.agents[1-self.playing_agent],
                        self.game.get_game_phase(self.agents[1-self.playing_agent])
...
def make_educated_or_random_move(node, agents, playing_agent, random_version=True):
    if random_version:
        ...
        highscore_node = None #will be assigned in very first 
        first = True
...
class MCTS:
    ...
    def _simulate(self, node):
        ...
        if res == None:
            ...
            '''print("replacing node: "+str(node.game.calculate_reward(
                        node.agents[1-node.playing_agent],
                        node.game.get_game_phase(node.agents[1-node.playing_agent]),
                        node.agents[node.playing_agent],
                        node.game.get_game_phase(node.agents[node.playing_agent])
                        ))+" with res: "+str(res.game.calculate_reward(
                        res.agents[1-res.playing_agent],
                        res.game.get_game_phase(res.agents[1-res.playing_agent]),
                        res.agents[res.playing_agent],
                        res.game.get_game_phase(res.agents[res.playing_agent])
                        )))'''
            node = res
        invert_reward = not invert_reward
...
class ComputerPlayer:
     ...
     def my_turn(self, game, agents, playing_agent):
                ...
                change = True
                round_cnt = round_cnt + 1
                return (change, round_cnt, game, agents, playing_agent)
...
class HumanPlayer:
    ...
    def my_turn(self, game, agents, playing_agent):
        ...
        if condition1 or condition2: 
            ...
            source = None
            target = None
        return (change, round_cnt, self.game, self.agents, self.playing_agent)
