...
class MainGame:
    ...
    def __init__(self, headlessMode=False):
    ...
    def play(self, player1, player2):
        ...
        while not done:
            if round_cnt % 2 == 0:
                source, target, change, round_cnt, self.game, self.agents, self.playing_agent = player1.my_turn(source, target, change, round_cnt, self.game, self.agents, self.playing_agent)
            else:
                source, target, change, round_cnt, self.game, self.agents, self.playing_agent = player2.my_turn(source, target, change, round_cnt, self.game, self.agents, self.playing_agent)
            ...
            if change:
                ...
                if self.game.check_end_game(self.agents[self.playing_agent]):
                    ...
                    if self.game.turns_without_mill >= 30:
                        ...
                        print("---WHITE (player2) WON---")
                        return 2
                    else:
                        print("---BLACK (player1) WON---")
                        return 1
...
def make_educated_or_random_move(node, agents, playing_agent, random_version=True):
    if random_version:
        make_random_move(node.game, agents, playing_agent)
        return None
    else:
        possibilities = node.find_children()
        highscore = 0 #will be assigned in very first
        highscore_node = [] #will be assigned in very first
        first = True

        for n in possibilities:
            if first:
                first = False
                highscore = n.game.calculate_reward(
                        n.agents[1-n.playing_agent],
                        n.game.get_game_phase(n.agents[1-n.playing_agent]),
                        n.agents[n.playing_agent],
                        n.game.get_game_phase(n.agents[n.playing_agent])
                        )
                highscore_node.append(n)
                continue

            calculated_reward = n.game.calculate_reward(
                        n.agents[1-n.playing_agent],
                        n.game.get_game_phase(n.agents[1-n.playing_agent]),
                        n.agents[n.playing_agent],
                        n.game.get_game_phase(n.agents[n.playing_agent])
                        )
            if calculated_reward > highscore:
                highscore_node.clear()
                highscore = calculated_reward
                highscore_node.append(n)
            elif calculated_reward == highscore:
                highscore_node.append(n)

        if highscore_node == None:
            print("whoopsie.... no possible moves? lol")

        return random.choice(highscore_node)
...
class MCTS:
    ...
    def do_rollout(self, node, prob_educatedFirst, prob_educatedSecond):
        ...
        reward = self._simulate(leaf, prob_educatedFirst, prob_educatedSecond)
    ...
    def _simulate(self, node, prob_educatedFirst, prob_educatedSecond):
        "Returns the reward for a random simulation (to completion) of `node`"
        invert_reward = True
        if node.is_terminal():
            reward = node.reward()
            return 1 - reward if invert_reward else reward
        if prob_educatedFirst == 0.0:
            random_var = 0
        else:
            random_var = random.random()
        if random_var < prob_educatedFirst:
            node = node.find_random_child(False)
        else:
            node = node.find_random_child(True)
        invert_reward = not invert_reward

        if node.is_terminal():
            reward = node.reward()
            return 1 - reward if invert_reward else reward
        if prob_educatedSecond == 0.0:
            random_var = 0
        else:
            random_var = random.random()
        if random_var < prob_educatedSecond:
            res = make_educated_or_random_move(node, node.agents, node.playing_agent,False)
        else:
            res = make_educated_or_random_move(node, node.agents, node.playing_agent,True)
        if res == None:
        ...
        while True:
            if node.is_terminal():
                reward = node.reward()
                return 1 - reward if invert_reward else reward
            res = make_educated_or_random_move(node, node.agents, node.playing_agent)
            if res == None:
                node.game.check_mode(*(node.agents))
                node.playing_agent = 1-node.playing_agent
            else:
...
class Node:
    ...
    def find_random_child(self, random_version=True):
        gamestate = clone_gamestate(self.game, self.agents, self.playing_agent)
        random_child = Node(gamestate[0], gamestate[1], gamestate[2])
        res = make_educated_or_random_move(random_child, random_child.agents, random_child.playing_agent, random_version)
        if res == None:
...
class ComputerPlayer:
    def __init__(self, exploration_weight=1, prob_educatedFirst=0, prob_educatedSecond=0, rollouts=1000, showThinking=True):
        self.tree = MCTS(exploration_weight)
        self.exploration_weight = exploration_weight
        self.prob_educatedFirst = prob_educatedFirst
        self.prob_educatedSecond = prob_educatedSecond
        self.rollouts = rollouts
        self.showThinking = showThinking

    def my_turn(self, source, target, change, round_cnt, game, agents, playing_agent):
                board = Node(game, agents, playing_agent)
                if self.showThinking:
                    for _ in tqdm(range(self.rollouts), desc="thinking...", ascii=False, ncols=75):
                        self.tree.do_rollout(board, self.prob_educatedFirst, self.prob_educatedSecond)
                else:
                ...
                game = board.game
                agents = board.agents
                playing_agent = board.playing_agent
                change = True
                round_cnt = round_cnt + 1
                return (source, target, change, round_cnt, game, agents, playing_agent)

    def get_details(self):
        return "Comp(exploration_weight="+str(self.exploration_weight)+", prob_educatedFirst="+str(self.prob_educatedFirst)+", prob_educatedSecond="+str(self.prob_educatedSecond)+", rollouts="+str(self.rollouts)+")"
...
class HumanPlayer:
    ...
    def my_turn(self, source, target, change, round_cnt, game, agents, playing_agent):
        ...
        if condition1 or condition2:
            ...
            source = None
            target = None
        return (source, target, change, round_cnt, self.game, self.agents, self.playing_agent)
